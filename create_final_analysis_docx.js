...
const quote = (text, source) => [
  new Paragraph({
    ...
    children: [
      new TextRun({ text: `"${text}"`, font: FONT, size: 20, italics: true, color: "444444" }),
    ],
  }),
  new Paragraph({
    spacing: { after: 160 },
    ...
    children: [
      new TextRun({ text: `— ${source}`, font: FONT, size: 18, color: "666666" }),
    ],
  }),
];
...
const heading1 = (text) =>
  new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: 32, bold: true, color: "1B3A5C" })],
  });
...
const heading3 = (text) =>
  new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: 22, bold: true, color: "3A6F9A" })],
  });
...
const emphasis = (text) =>
  new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: 20, bold: true, color: "555555" })],
  });
...
const bibleVerse = (ref, text) =>
  new Paragraph({
    ...
    children: [
      new TextRun({ text: `${ref}`, font: FONT, size: 20, bold: true, color: "1B3A5C" }),
      new TextRun({ text: ` — ${text}`, font: FONT, size: 20, color: "555555" }),
    ],
  });
...
const subHeading = (text) =>
  new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: 22, bold: true, color: "1B3A5C" })],
  });

// ── Build reason section ──
function buildReason(num, title, description, quotes, emphasisText, bibleVerses) {
  const children = [
  ...
  if (bibleVerses && bibleVerses.length > 0) {
    children.push(subHeading("→ 관련 성경 구절"));
  ...
  sections: [{
    ...
    headers: {
      default: new Header({
        children: [new Paragraph({
          alignment: AlignmentType.RIGHT,
          children: [new TextRun({ text: "MiniMax 분석 | 조용기牧师 설교 | 생각이 왜 중요한가", font: FONT, size: 16, color: "999999" })],
        })],
      }),
    },
    footers: {
    ...
    children: [
      ...
      new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing: { after: 200 },
        children: [new TextRun({ text: "MiniMax AI 분석 결과", font: FONT, size: 28, bold: true, color: "666666" })],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing: { after: 200 },
        children: [new TextRun({ text: "조용기牧师 설교 분석", font: FONT, size: 44, bold: true, color: "1B3A5C" })],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing: { after: 400 },
      ...
      new Paragraph({
        ...
        children: [new TextRun({ text: "분석 도구: MiniMax AI", font: FONT, size: 22, color: "555555" })],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER, spacing: { after: 60 },
        children: [new TextRun({ text: "분석 범위: original_sermon 폴더 1,347편 전체 설교 전수 조사 (1981~2011)", font: FONT, size: 20, color: "555555" })],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER, spacing: { after: 60 },
        children: [new TextRun({ text: "핵심 설교: 30여 편 전수 검토", font: FONT, size: 20, color: "555555" })],
      }),
      new Paragraph({ children: [new PageBreak()] }),

      // ─── REASON 1 ───
      ...buildReason(1, "생각은 인간의 위대성이 놓여 있는 자리이다",
        "조용기 목사는 인간의 참된 위대함이 육체가 아닌 '생각'에 있다고 일관되게 가르쳤다. 육체적으로 인간은 호랑이나 코끼리보다 약하지만, 생각을 통해 우주의 끝까지, 바다 깊이까지, 원자의 세계까지 미치는 존재라는 것이다. 파스칼의 \"생각하는 갈대\" 비유를 자주 인용하면서, 몸은 연약하지만 그 속에 있는 생각은 위대한 거인이라고 강조하였다.",
        [
      ...
      // ─── REASON 2 ───
      ...buildReason(2, "생각이 곧 그 사람 자체이며, 미래를 결정한다",
        '조용기 목사의 가장 근본적인 명제 중 하나는 "생각이 곧 그 사람"이라는 것이다. 병든 생각은 병든 인간을, 가난한 생각은 가난한 인간을, 승리의 생각은 승리하는 인간을 만든다고 반복적으로 설교했다. 자신의 생각대로 된다는 법칙을 인간 존재의 근본 원리로 제시하면서, 생각을 바꾸면 미래도 바뀐다는 실용적 적용을 가르쳤다.',
        [
          { text: "인간이 스스로 생각이 약하면 그는 약한 인간이 되고 생각이 병들면 병은 인간이 되고 생각이 악한 사람이면 악한 인간이 되어 버리고 마는 것입니다.", source: '1981.6.21. "나, 나의 생각"' },
          ...
          { text: "사람은 자신이 생각하는 모습대로 되는 것이다. 지금 자신의 모습은 자신의 생각에서 비롯된 것이다. 내일 다른 위치에 있고자 한다면 자신의 생각을 바꾸면 된다.", source: '2006.2.5. "내적 저항력"' },
        ],
      ...
      // ─── REASON 3 ───
      ...buildReason(3, "생각은 하나님께서 역사하시는 통로(그릇)이다",
        '조용기 목사는 하나님이 인간의 생각을 통해 역사하신다고 일관되게 가르쳤다. 에베소서 3:20을 근거로 "구하는 것이나 생각하는 것"에 넘치도록 하시는 하나님이시므로, 생각이 하나님 역사의 그릇이 된다는 것이다. 중요한 것은, 부정적 생각으로 채워진 그릇에는 하나님도 응답하실 수 없다는 점이다.',
        [
          { text: "하나님도 여러분의 생각을 통해서 역사합니다. 성경에는 우리의 온갖 구하는 것이나 생각하는 것에 넘치도록 능히 하실 하나님이라고 말씀하고 있는 것입니다.", source: '1981.6.21. "나, 나의 생각"' },
          { text: "하나님은 우리의 생각을 통해서 역사 하시는 것입니다. 우리 생각을 통해서 하나님을 뵙고 하나님은 우리 생각을 통해서 나타나시는 것입니다.", source: '2007.11.25. "없는 것을 있는 것 같이"' },
        ...
        [
          ...
          { ref: "시편 81:10", text: "네 입을 넓게 열라 내가 채우리라" },
        ]
      ),

      // ─── REASON 4 ───
      ...buildReason(4, "생각은 4차원의 영적 세계를 움직이는 힘이다",
        "'4차원의 영성'은 조용기牧师 신학의 핵심이다. 3차원(물질·시간·공간)을 지배하는 4차원의 세계가 있으며, 그 4차원의 구성요소가 바로 생각, 꿈(상상), 믿음, 입술의 고백이라고 가르쳤다. 높은 차원이 낮은 차원을 다스린다는 원리에 따라, 4차원의 생각과 믿음이 3차원의 물질 세계를 변화시킨다는 논리 구조를 세웠다.",
        [
          ...
          { text: "4차원의 세계가 생각을 통해서 꿈을 통해서 믿음을 통해서 말씀의 선언을 통해서 나타나면 환경인 3차원의 세계가 변화되는 것입니다.", source: '2007.11.25. "없는 것을 있는 것 같이"' },
        ...
        [
          ...
          { ref: "창세기 1:1-3", text: "하나님이 빛이 있으라 하시니 빛이 있었다" },
          { ref: "로마서 4:17", text: "없는 것을 있는 것으로 부르시는 이시니라" },
        ]
      ),

      // ─── REASON 5 ───
      ...buildReason(5, "생각을 다스리는 자가 운명과 환경을 다스린다",
        "생각이 감정을 격동시키고, 의지를 지배하며, 결국 운명과 환경 전체를 만들어낸다는 것이 조용기牧師の 핵심 논리 구조이다. 눈에 보이지 않는 생각을 다스리는 것이 눈에 보이는 운명을 다스리는 것이라는 실용적 원리를 강조하면서, 마음에 있는 것이 현실로 나타난다고 가르쳤다.",
        [
          ...
          { text: "자기의 인생을 매니지하고 다스리고 돌보는 일을 자기의 생각을 다스리므로 자기 소원과 꿈을 다스리므로 자기의 믿음을 다스리므로 자기 입술의 말을 다스리므로 자기의 운명을 다스릴 수가 있는 것입니다.", source: '2010.11.7. "사차원의 삶"' },
      ...
      // ─── REASON 6 ───
      ...buildReason(6, "생각의 변화가 곧 회개(메타노이아)이며 구원의 시작이다",
        "조용기 목사는 '회개'의 헬라어 원어 '메타노이아'를 생각의 전환으로 해석하며, 참된 회개란 단순한 뉘우침이 아니라 생각 전체가 바뀌는 것이라고 가르쳤다. 천국도 의식이 바뀌어야 들어올 수 있으며, 마음을 새롭게 함으로 변화받는 것이 구원의 핵심이라고 설교했다.",
        [
          { text: "예수님께서는 말씀하시기를 '회개하라 천국이 가까이 왔다. 마음의 생각을 바꾸라 의식을 바꾸라 천국이 손 닫는데 와 있다'고 말한 것입니다. 천국이 아무리 손 닫는데 와 있어도 의식이 바꾸어지지 아니하면 우리 속에 들어오지 못합니다.", source: '1981.8.30. "의식혁명"' },
          { text: "회개라는 것은 '메탈로얄' 헬라어로써 우리의 죄악에서 우리 스스로를 돌이킬 뿐 아니라 우리의 생각과 우리의 감정조차도 모두다 변화시키는 것이 바로 회개인 것입니다. 마음을 고쳐먹는 것입니다.", source: '1983.11.27. "긍정적인 생각의 축복"' },
      ...
      // ─── REASON 7 ───
      ...buildReason(7, "생각은 눈에 보이지 않는 가장 큰 재산이자 자원이다",
        "조용기 목사는 생각을 눈에 보이지 않지만 황금보화로도 바꿀 수 없는 최고의 재산이자 자원이라고 반복해서 강조했다. 인간이 자본이 고갈되어도 아이디어가 있는 한 멸망하지 않으며, 아이디어가 고갈될 때 비로소 멸망한다고 가르쳤다.",
        [
          ...
          { text: "우리에게 주신 생각은 황금보화로도 바꿀 수 없는 가장 귀한 재산이요 자원입니다.", source: '1984.11.11. "생각을 바꿔라"' },
          { text: "인간이 자본고갈로 절대로 멸망하지 않습니다. 인간의 아이디어가 고갈할 때 인간은 멸망하고 마는 것입니다.", source: '1984.11.11. "생각을 바꿔라"' },
        ],
        '"눈에 안보이는 가장 큰 재산", "황금보화로도 바꿀 수 없는 자원", "생각의 자원이 고갈하면 멸망"',
        [
          { ref: "잠언 4:23", text: "지킬만한 것보다 네 마음을 지켜라 생명의 근원이 이에서 남이니라" },
      ...
      // ─── REASON 8 ───
      ...buildReason(8, "하나님의 형상대로 지음받은 생각은 창조적 능력을 지닌다",
        "하나님이 인간을 자신의 형상대로 지으셨기에, 인간의 생각에는 하나님처럼 창조하고 지배할 수 있는 능력이 내재되어 있다는 것이다. 하나님은 영(4차원)이시고, 인간도 하나님의 형상대로 영이므로, 인간도 하나님처럼 생각하고 꿈꾸고 믿고 말할 수 있는 것이다.",
        [
          ...
          { text: "여러분은 하나님의 손으로 지었기 때문에 하나님이 사차원적인 영인 것이므로 여러분도 영입니다. 그렇기 때문에 여러분도 하나님처럼 생각하고 꿈꾸고 믿고 말할 수 있는 것입니다.", source: '2010.11.7. "사차원의 삶"' },
        ],
      ...
      // ─── REASON 9 ───
      ...buildReason(9, "생각이 바뀌면 꿈이 바뀌고, 믿음이 바뀌고, 말이 바뀌고, 운명이 바뀐다",
        "조용기 목사는 생각 → 꿈 → 믿음 → 말 → 환경/운명이라는 연쇄 구조를 거의 모든 설교에서 반복했다. 이 연쇄의 출발점이 바로 '생각'이기에 생각이 중요하다는 것이다. 생각을 바꾸면 그 다음의 모든 단계가 연쇄적으로 변화한다는 실용적 원리를 가르쳤다.",
        [
          ...
          { text: "하나님 말씀을 통하여 생각을 바로잡고 꿈을 바로잡고 믿음을 바로가지고 바른 승리의 긍정적인 말을 하기 시작하면 운명이 달라지는 것입니다.", source: '2005.9.11. "패배자는 설 곳이 없다"' },
      ...
      // ─── REASON 10 ───
      ...buildReason(10, "부정적인 생각은 마귀의 역사이며, 영적 파괴의 시작이다",
        "조용기 목사는 부정적·파괴적·절망적 생각이 단순한 심리적 문제가 아니라 마귀가 인간의 생각에 부패의 씨를 심은 결과라고 가르쳤다. 열등의식, 피해의식, 가난의식, 패배의식 등 부정적인 의식이 인격을 점령하면 그 사람의 삶 전체가 부정적이 된다는 것이다.",
        [
          ...
          { text: "마귀는 우리에게 낙심하고 절망하도록 여러 가지 부정적인 생각을 넣어줍니다.", source: '2009.8.2. "기도와 낙망"' },
        ],
        '"부정적 생각은 마귀의 선물"',
        [
      ...
      // ─── REASON 11 ───
      ...buildReason(11, "바라봄(상상)을 통해 생각이 믿음을 잉태시킨다",
        "'바라봄의 법칙'은 조용기牧师 신학에서 생각과 믿음을 연결하는 핵심 원리이다. 말씀을 듣고 이루어진 모습을 마음에 생생하게 그려보면(상상하면), 그 상상을 통해 믿음이 생겨나고, 그 믿음이 현실을 창조한다는 것이다. 꿈이 없는 곳에는 믿음도 없다.",
        [
          ...
          { text: "여러분의 마음의 꿈이 비로서 여러분 마음속에 믿음을 만들어 내는 것입니다. 사람이 믿음이 약한 사람은 꿈이 없는 사람이요, 꿈을 안꾸는 사람은 믿음도 가질수가 없는 것입니다.", source: '1988.6.12. "마음의 변화"' },
      ...
      // ─── REASON 12 ───
      ...buildReason(12, "생각이 몸의 건강과 질병을 좌우한다",
        "조용기 목사는 생각이 영적·심리적 영역만이 아니라 육체적 건강에도 직접적 영향을 미친다고 가르쳤다. 현대 뇌과학의 연구 결과를 인용하여, 스트레스와 부정적 감정이 노르아드레날린이라는 독성 호르몬을 분비하고, 긍정적 생각이 베타-엔돌핀이라는 유익한 호르몬을 분비한다고 설교했다.",
        [
          ...
          { text: "생각이 우리 몸을 파괴하고 늙게 하는 효소를 몸에서 생겨나게 하고 우리를 건강하고 장수하게 하는 효소를 생겨나게도 하는 것입니다. 생각이 우리 몸도 좌우하고 환경도 좌우하고 운명을 좌우하는 것입니다.", source: '2007.11.25. "없는 것을 있는 것 같이"' },
          { text: "마음에 염려, 근심, 불안, 초조, 절망, 부정적이고 우울한 마음을 가지고 있으면 온갖 병이 다 와요.", source: '2010.9.19. "네 마음을 지켜라"' },
        ],
      ...
      // ─── REASON 13 ───
      ...buildReason(13, "말씀이 생각을 변화시키고, 변화된 생각이 삶을 변화시킨다",
        "조용기 목사는 생각 변화의 유일한 근거이자 원천이 하나님의 말씀이라고 가르쳤다. 이유 없는 긍정은 허무하며, 십자가의 구속과 말씀이 생각을 바꿀 조건과 이유가 된다는 것이다. 성경을 읽으면 하나님의 생각이 내 생각에 접붙여지고, 그 생각이 뿌리를 내릴 때 하나님이 역사하신다.",
        [
          ...
          { text: "성경이 왜 우리에게 중요하냐면 성경이 여러분의 생각을 바꿀 수 있는 하나님의 말씀인 것입니다. 소설책 읽는다고 생각이 바꿔지나요? 하나님 말씀을 읽으면 여러분 생각이 바꿔져요. 생각이 달라지면 운명이 달라져요.", source: '2010.4.25. "감사와 찬송의 힘"' },
        ],
        '"말씀이 생각을 변화시킨다", "하나님의 생각이 내 생각에 접붙여진다", "십자가가 생각 변화의 근거"',
        [
          { ref: "요한복음 8:32", text: "진리를 알지니 진리가 너희를 자유케 하리라" },
          { ref: "로마서 10:17", text: "믿음은 들음에서 나며 들음은 그리스도의 말씀으로 말미암았느니라" },
          { ref: "여호수아 1:8", text: "이 율법책을 네 입에서 떠나지 말게 하며 주야로 그것을 묵상하여" },
        ]
      ),

      // ─── REASON 14 ───
      ...buildReason(14, "잘못된 생각은 개인뿐 아니라 가정·사회·국가를 파멸시킨다",
        "조용기 목사는 생각의 영향력을 개인 차원을 넘어 사회·국가·민족 차원으로 확장하여 가르쳤다. 가나안 12정탐꾼 사건을 반복 인용하면서, 10명의 부정적 생각이 300만 이스라엘 민족 전체의 패망을 가져왔고, 여호수아와 갈렙의 긍정적 생각은 가나안 입성이라는 승리를 가져왔다고 가르쳤다.",
        [
          ...
          { text: "우리가 어린이들에게 생각하는 것 탐험가에게 하든 어떤 씨를 뿌리든 그 씨대로 자라는 것입니다. 마땅히 아이들에게 하나님을 경외하고 하나님을 통해서 세상을 크게 바꿀 수 있는 크고 위대한 사람이 되라는 생각의 씨를 심어주어야 합니다.", source: '1981.6.21. "나, 나의 생각"' },
          { text: "꿈이 없는 백성은 망한다.", source: '1982.6.27. "꿈과 승리의 생활" (잠언 29:18 인용)' },
        ],
        '"꿈이 없는 백성은 망한다"',
        [
          { ref: "민수기 13-14장", text: "12정탐꾼 사건" },
      ...
      body("조용기牧师의 1,347편 설교를 전수 조사한 결과, \"생각이 왜 중요한가\"에 대한 그의 가르침은 다음과 같이 다섯 가지 핵심으로 요약된다."),
      new Paragraph({ spacing: { after: 80 }, children: [] }),

      heading3("첫째, 생각은 인간 존재의 본질이다."),
      body("인간의 위대함은 육체가 아니라 생각에 있으며, \"그 마음의 생각이 어떠하면 그 사람도 그러하다\"(잠 23:7). 생각이 곧 그 사람 자체이며, 그 사람의 현재와 미래를 결정한다."),

      heading3("둘째, 생각은 하나님 역사의 통로이다."),
      body("하나님은 인간의 \"구하는 것이나 생각하는 것\"(엡 3:20)에 넘치도록 역사하시므로, 생각이 곧 하나님의 역사가 담기는 그릇이다. 부정적 생각으로 채워진 그릇에는 하나님도 역사하실 수 없다."),

      heading3("셋째, 생각은 4차원의 영적 세계에 속한다."),
      body("3차원의 물질 세계를 지배하는 것은 4차원의 영적 세계이며, 생각은 꿈·믿음·고백과 함께 4차원의 핵심 요소이다. 따라서 생각을 다스리는 것이 곧 운명과 환경을 다스리는 것이다."),

      heading3("넷째, 생각의 변화는 말씀에 근거해야 한다."),
      body("이유 없는 긍정은 허무하며, 십자가의 구속과 하나님의 약속의 말씀이 생각 변화의 유일한 근거이다. 회개(메타노이아) 자체가 생각의 전환이며, 말씀을 통해 하나님의 생각이 내 생각에 접붙여질 때 참된 변화가 일어난다."),

      heading3("다섯째, 생각의 변화는 연쇄 반응을 일으킨다."),
      body("생각이 바뀌면 꿈(상상)이 바뀌고, 꿈이 바뀌면 믿음이 생겨나고, 믿음이 생기면 입술의 고백이 달라지고, 고백이 달라지면 운명과 환경이 달라진다. 이 연쇄의 출발점이 바로 생각이기에, 조용기牧师는 30년에 걸친 설교에서 한결같이 \"생각을 바꾸라\"고 외쳤다."),
      ...
      new Paragraph({
        ...
        spacing: { before: 200 },
        children: [
          new TextRun({ text: "본 분석은 MiniMax AI를 통해 original_sermon 폴더의 1,347편 설교 텍스트를 전수 분석하여 작성되었습니다.", font: FONT, size: 18, color: "888888" }),
        ],
      }),
    ],
  }],
...
Packer.toBuffer(doc).then((buffer) => {
  const outPath = "d:\\AI_PROJECT\\_new_drcho_sermon_all\\MiniMax_분석_생각이_왜_중요한가.docx";
